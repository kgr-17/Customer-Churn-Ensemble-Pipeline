...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train multi-seed CV blend (CatBoost + LightGBM) and create Kaggle submission"
    )
    ...
    parser.add_argument(
        "--submission",
        default="outputs/submission_blend_v3_multiseed.csv",
        help="Output submission CSV",
    )
    parser.add_argument(
        "--metrics",
        default="outputs/metrics_blend_v3_multiseed.json",
    ...
    parser.add_argument(
        ...
        help="Comma-separated CV seeds (example: 42,2024,3407)",
    )
...
def train_catboost_cv(
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
        ...
        params: dict[str, object] = {
            ...
            "train_dir": f"catboost_info/v3_seed_{seed}_fold_{fold}",
        }
        if task_type == "GPU":
        ...
        try:
            ...
            raise RuntimeError(f"CatBoost failed on {task_type} at seed {seed}, fold {fold}: {exc}") from exc
...
def train_lgbm_cv(
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_lgb, y), start=1):
        ...
        model.fit(
            ...
            callbacks=[early_stopping(300), log_evaluation(verbose_every)],
        )
        ...
        fold_auc = float(roc_auc_score(y_va, oof_fold))
        fold_scores.append(fold_auc)
        print(f"[Seed {seed}] LightGBM fold {fold} AUC: {fold_auc:.6f}")
...
def main() -> None:
    ...
    lgb_oof_all: list[np.ndarray] = []
    cat_test_all: list[np.ndarray] = []
    lgb_test_all: list[np.ndarray] = []
    ...
    for seed in seeds:
        ...
        cat_oof, cat_test, cat_fold_scores = train_catboost_cv(
            X, y, X_test, cat_features, skf, seed, args.verbose_every, cat_task_type, args.gpu_devices
        )
        lgb_oof, lgb_test, lgb_fold_scores = train_lgbm_cv(
            X, y, X_test, cat_features, skf, seed, args.n_jobs, args.verbose_every
        )

        cat_auc = float(roc_auc_score(y, cat_oof))
        lgb_auc = float(roc_auc_score(y, lgb_oof))

        seed_blend_weight, seed_blend_auc = optimize_blend_weight(y, cat_oof, lgb_oof)
        seed_blend_test = seed_blend_weight * cat_test + (1.0 - seed_blend_weight) * lgb_test
        seed_blend_oof = seed_blend_weight * cat_oof + (1.0 - seed_blend_weight) * lgb_oof

        cat_oof_all.append(cat_oof)
        lgb_oof_all.append(lgb_oof)
        cat_test_all.append(cat_test)
        lgb_test_all.append(lgb_test)
        ...
        per_seed_metrics.append(
            {
                ...
                "lightgbm_fold_auc": lgb_fold_scores,
                "catboost_oof_auc": cat_auc,
                "lightgbm_oof_auc": lgb_auc,
                "seed_blend_weight_catboost": seed_blend_weight,
                "seed_blend_weight_lightgbm": 1.0 - seed_blend_weight,
                "seed_blend_oof_auc": seed_blend_auc,
            }
        )

    cat_oof_mean = np.mean(np.vstack(cat_oof_all), axis=0)
    lgb_oof_mean = np.mean(np.vstack(lgb_oof_all), axis=0)
    cat_test_mean = np.mean(np.vstack(cat_test_all), axis=0)
    lgb_test_mean = np.mean(np.vstack(lgb_test_all), axis=0)

    cat_oof_auc_mean = float(roc_auc_score(y, cat_oof_mean))
    lgb_oof_auc_mean = float(roc_auc_score(y, lgb_oof_mean))

    model_avg_weight_cat, model_avg_blend_auc = optimize_blend_weight(y, cat_oof_mean, lgb_oof_mean)
    model_avg_test_blend = model_avg_weight_cat * cat_test_mean + (1.0 - model_avg_weight_cat) * lgb_test_mean
    ...
    metrics = {
        ...
        "lightgbm_oof_auc_seed_mean": lgb_oof_auc_mean,
        "model_avg_then_optimize": {
            "catboost_weight": model_avg_weight_cat,
            "lightgbm_weight": 1.0 - model_avg_weight_cat,
    ...
    print(f"LightGBM mean-seed OOF AUC: {lgb_oof_auc_mean:.6f}")
